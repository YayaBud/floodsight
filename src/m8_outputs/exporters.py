...
def export_kml(gdf: gpd.GeoDataFrame, out_dir: str | Path,
    ...
    kml = simplekml.Kml(name="FloodSense — Priority Ranking")
...
def export_cap_json(
    ...
    cap = {
        "identifier": f"FloodSense-{run_id or dam_name}-{now[:10]}",
        "sender":     "FloodSense",
        "sent":        now,
        "status":     "Exercise",
        "msgType":    "Alert",
        "scope":      "Restricted",
        "note":       (
            "GENERATED BY FloodSense (SIH26161). "
        ...
        "info": [{
            ...
            "description": (
                f"FloodSense exercise output for {dam_name}. "
